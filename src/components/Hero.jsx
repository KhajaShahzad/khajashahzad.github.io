  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const getLayout = () => {
        ...
        const startW = desktop ? Math.min(vw * 0.30, 470) : Math.min(vw * 0.64, 400);
        const startH = startW * 1.28;
        const startLeft = desktop ? Math.min(vw * 0.61, vw - startW - 40) : (vw - startW) / 2;
        const startTop = desktop ? Math.max(88, vh * 0.12) : Math.max(105, vh * 0.20);
...
gsap.set(straightPortraitRef.current, {
  opacity: eased,
          ...
          if (t < 0.999) {
            ...
            const docLeft = m.endLeft + window.scrollX;
            const docTop = m.endTop + window.scrollY;
